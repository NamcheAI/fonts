...
DEFAULT_PACKAGE = SCRIPT_DIR.parent / "sources" / "NamcheShadowSans.glyphspackage"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        action="append",
        help="Path to .glyphspackage (repeatable). Default: sources/NamcheShadowSans.glyphspackage",
    )
    parser.add_argument(
        "--strong",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Rewrite familyName and VF fileName (e.g. "Namche Shadow Sans")',
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    for package in packages:
        ...
        if args.family_name:
            file_prefix = re.sub(r"[^A-Za-z0-9]", "", args.family_name)
            new_text = re.sub(
